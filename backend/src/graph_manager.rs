...
use crate::types::{EngineConfig, LegDetail, Opportunity, OrderBookHealth};
use chrono::Utc;
...
impl PersistentGraph {
    ...
    pub fn update_pair(
        ...
        let (bid, ask, valid) = match (&price, &order_book) {
            (Some(p), Some(book)) => {
                // Validate order book using HFT thresholds
                let has_depth = book.bids.len() >= crate::types::MIN_ORDERBOOK_DEPTH
                    && book.asks.len() >= crate::types::MIN_ORDERBOOK_DEPTH;
                let is_fresh = book.staleness_ms() < crate::types::MAX_ORDERBOOK_STALENESS_MS;
                ...
                let reasonable_spread = spread_pct >= 0.0 && spread_pct < crate::types::MAX_SPREAD_PCT;
    ...
    pub fn update_health_from_cache(&self, cache: &OrderBookCache) {
        // Health check thresholds from HFT config
        let min_depth: usize = crate::types::MIN_ORDERBOOK_DEPTH;
        let max_staleness_ms: i64 = crate::types::MAX_ORDERBOOK_STALENESS_MS;
        let max_spread_pct: f64 = crate::types::MAX_SPREAD_PCT;
