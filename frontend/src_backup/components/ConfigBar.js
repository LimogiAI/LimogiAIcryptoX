...
export function ConfigBar() {
  ...
  const [configStatus, setConfigStatus] = useState(null); // Configuration status from API
  ...
  const fetchData = useCallback(async () => {
    try {
      const [configRes, statusRes, feesRes, configStatusRes] = await Promise.all([
        api.getLiveConfig(),
        api.getLiveStatus().catch(() => null),
        api.getKrakenFees().catch((err) => ({ success: false, error: err.message })),
        api.getConfigurationStatus().catch(() => null)
      ]);
      setConfig(configRes.config);
      if (statusRes) setStatus(statusRes);
      if (configStatusRes?.status) setConfigStatus(configStatusRes.status);
  ...
  const getConfigValue = (key) => {
    ...
    if (config?.[key] !== undefined && config[key] !== 0 && config[key] !== '') return config[key];
    return null; // No fallbacks - return null if not set
  };

  const currentTradeAmount = getConfigValue('trade_amount');
  const currentThreshold = getConfigValue('min_profit_threshold');
  const currentBaseCurrency = getConfigValue('base_currency');
  ...
  // Configuration status - can only start if fully configured
  const isConfigured = configStatus?.is_configured ?? false;
  const canStartEngine = configStatus?.can_start_engine ?? false;
  const missingFields = configStatus?.missing_fields ?? [];
  const configWarnings = configStatus?.warnings ?? [];
  ...
  // Helper to format values or show "Not Set"
  ...
  return (
    <div className="config-bar">
      {/* Configuration Status Banner - show when not configured */}
      {!isConfigured && (
        <div className="config-banner config-banner-warning">
          <div className="config-banner-icon">⚠️</div>
          <div className="config-banner-content">
            <strong>Configuration Required</strong>
            <p>Please configure the following before starting the engine:</p>
      ...
      {isConfigured && configWarnings.length > 0 && (
        <div className="config-banner config-banner-info">
          <div className="config-banner-icon">ℹ️</div>
          <div className="config-banner-content">
            <strong>Configuration Warnings</strong>
            <ul className="warning-fields-list">
      ...
      <div className="active-config-summary">
        <div className="summary-item">
          ...
          <span className="summary-value">
            {formatValue(config?.trade_amount, (v) => `$${v}`)}
          </span>
        </div>
        ...
        <div className="summary-item">
          ...
          <span className="summary-value">
            {formatValue(config?.min_profit_threshold, (v) => `${(v * 100).toFixed(2)}%`)}
          </span>
        </div>
        <div className="summary-divider">|</div>
        <div className="summary-item">
          <span className="summary-icon">💵</span>
          <span className="summary-label">Start Currency:</span>
          <span className="summary-value">
            {formatValue(config?.base_currency, getBaseCurrencyDisplay)}
          </span>
        </div>
        <div className="summary-divider">|</div>
        <div className="summary-item">
          ...
          <span className="summary-value">
            {config?.max_daily_loss > 0 && config?.max_total_loss > 0
        ...
        <div className="trading-control">
          {isEnabled ? (
            ...
            <button
              ...
              disabled={tradingLoading || isCircuitBroken || !canStartEngine}
              title={!canStartEngine ? 'Complete configuration first' : ''}
            >
              {tradingLoading ? '...' : (!canStartEngine ? '⚙️ Configure First' : '🟢 START Trading')}
            </button>
          )}
          {!canStartEngine && !isEnabled && (
            <span className="config-required-hint">Configuration required</span>
          )}
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowEnableModal(false)}>
          <div className="enable-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-content">
              ...
              <ul>
                <li>Trade Amount: <strong>${config?.trade_amount}</strong></li>
                <li>Min Profit Threshold: <strong>{config?.min_profit_threshold ? `${(config.min_profit_threshold * 100).toFixed(2)}%` : 'Not Set'}</strong></li>
                <li>Start Currency: <strong>{config?.base_currency || 'Not Set'}</strong></li>
                <li>Daily Loss Limit: <strong>${config?.max_daily_loss}</strong></li>
                <li>Total Loss Limit: <strong>${config?.max_total_loss}</strong></li>
              </ul>
