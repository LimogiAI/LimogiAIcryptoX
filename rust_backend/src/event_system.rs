...
pub struct EventDrivenScanner {
    ...
    // Channel for saving profitable opportunities to database
    opportunity_save_tx: Arc<RwLock<Option<mpsc::UnboundedSender<Opportunity>>>>,
    
    // Channel for auto-execution (sends profitable opportunities to TradingEngine)
    auto_exec_tx: Arc<RwLock<Option<mpsc::UnboundedSender<Opportunity>>>>,
}
...
impl EventDrivenScanner {
    pub fn new(
        ...
        Self {
            ...
            opportunity_save_tx: Arc::new(RwLock::new(None)),
            auto_exec_tx: Arc::new(RwLock::new(None)),
        }
    }
    ...
    /// Create opportunity save channel for database persistence
    pub fn create_opportunity_save_channel(&self) -> mpsc::UnboundedReceiver<Opportunity> {
        let (tx, rx) = mpsc::unbounded_channel();
        *self.opportunity_save_tx.write() = Some(tx);
        rx
    }

    /// Create auto-execution channel for TradingEngine to receive profitable opportunities
    pub fn create_auto_exec_channel(&self) -> mpsc::UnboundedReceiver<Opportunity> {
        let (tx, rx) = mpsc::unbounded_channel();
        *self.auto_exec_tx.write() = Some(tx);
        rx
    }
    ...
    fn try_scan(&self) {
        ...
        *self.last_scan_time.write() = Instant::now();
        ...
        // Send profitable opportunities to save channel (for database persistence)
        if let Some(ref tx) = *self.opportunity_save_tx.read() {
            for opp in opportunities.iter().filter(|o| o.is_profitable) {
                if tx.send(opp.clone()).is_err() {
                    debug!("Failed to send opportunity to save channel");
                    break;
                }
            }
        }
            ...
            if let Some(ref tx) = *self.auto_exec_tx.read() {
                    ...
                    .max_by(|a, b| a.net_profit_pct.partial_cmp(&b.net_profit_pct).unwrap_or(std::cmp::Ordering::Equal))
                {
                    if tx.send(best_opp.clone()).is_ok() {
                        debug!("Sent opportunity for auto-execution: {} ({:.3}%)", best_opp.path, best_opp.net_profit_pct);
                    }
                }
            }
        }
    ...
    fn try_auto_execute(&self, opportunities: &[Opportunity]) {
        ...
        for opp in profitable.iter().take(1) {  // Only try best one per scan cycle
            ...
            let exec_opp = Opportunity {
                ...
                fee_source: opp.fee_source.clone(),
                legs_detail: opp.legs_detail.clone(),
            };
