...
impl ConfigManager {
    pub fn new(config: EngineConfig) -> Self {
        ...
        info!("Fee rate: {:.2}% (source: {})", config.fee_rate * 100.0, config.fee_source);
    ...
    pub fn update_config(
        ...
        if let Some(fee) = fee_rate {
            config.fee_rate = fee;
            // When fee is updated via API, it's from live Kraken data
    ...
    /// Get current fee source
    pub fn get_fee_source(&self) -> String {
        self.config.read().fee_source.clone()
    }
